...
class TestConvertFilename:
    ...
    def test_convert_file_names_with_dirs(self, tmpdir):
        result = FileParser.convert_filename(path.join(tmpdir, "Very Good Game (Track 1) PAL.raw"))
        assert(result == path.join(tmpdir, "track01.raw"))

        result = FileParser.convert_filename(path.join(tmpdir, "track racing game 2-track5-(region and language information).bin"))
        assert(result == path.join(tmpdir, "track05.bin"))

        result = FileParser.convert_filename(path.join(tmpdir, "It's Incredible! - Worldwide?! (Europe) (En,Fr,De,Es) (Track0008).bin"))
        assert(result == path.join(tmpdir, "track08.bin"))

        result = FileParser.convert_filename(path.join(tmpdir, "_THE_GAME_TRACK_4_.bin"))
        assert(result == path.join(tmpdir, "track04.bin"))

        result = FileParser.convert_filename(path.join(tmpdir, "Sometimes.People.Do.This.Track.12.raw"))
